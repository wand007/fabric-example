...
pushd /opt/gopath/src/github.com/hyperledger/chaincode/chaincode/marbles02_private/go/
...
peer lifecycle chaincode approveformyorg --tls true --cafile $CORE_PEER_TLS_ROOTCERT_FILE --channelID $CHANNEL_NAME --name marbles02_private --version 1 --init-required --package-id $CC_PACKAGE_ID --sequence 1 --waitForEvent
...
# 链码数据查询
peer chaincode query -n marbles02_private -C mychannel -c '{"Args":["query","a"]}'
